...
class product {
    ...
    public function ModProd($pid,$name,$desc,$price,$file)
    {
      try {
          $sql ="UPDATE products SET `name`= :name , `description`=:description, `price`=:price, `file`=:file WHERE `pid`=:pid";
          $result = $this->ExecutionReq($sql);
          $result-> bindparam(":name",$name);
          $result-> bindparam(":description",$desc);
          $result-> bindparam(":price",$price);
          $result-> bindparam(":file",$file);
          $result-> bindparam(":pid",$pid);
          $result->execute();
